...
{
  std::cout << "glgrib_options_coastlines::init" << std::endl;
  ...
  for (int pass = 0; pass < 2; pass++)
    {
      int count = 0;
      for (int i = 0; i < opts.path.size (); i++)
        {
          FILE * fp = fopen (glgrib_resolve (opts.path[i]).c_str (), "r");

          if (fp == NULL)
            std::runtime_error (std::string ("Cannot open coastline data"));
